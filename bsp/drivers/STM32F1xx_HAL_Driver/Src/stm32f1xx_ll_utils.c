                                         ...
                                         LL_UTILS_ClkInitTypeDef *UTILS_ClkInitStruct)
{
  ...
  status = UTILS_PLL_HSE_ConfigSystemClock(HSEFrequency, HSEBypass, UTILS_PLLInitStruct,
                                           UTILS_ClkInitStruct);
  ...
  {
    /* Configure PLL */
    LL_RCC_PLL_ConfigDomain_SYS((LL_RCC_PLLSOURCE_HSE | UTILS_PLLInitStruct->Prediv),
                                          ...
                                          LL_UTILS_ClkInitTypeDef *UTILS_ClkInitStruct)
{
  ...
  if (status == SUCCESS)
  {
    ...
    LL_RCC_PLL_ConfigDomain_SYS((LL_RCC_PLLSOURCE_PLL2 | UTILS_PLLInitStruct->Prediv),
                                UTILS_PLLInitStruct->PLLMul);
...
#if defined(RCC_CFGR2_PREDIV1)
  pllfreq = __LL_RCC_CALC_PLLCLK_FREQ(PLL_InputFrequency / (UTILS_PLLInitStruct->Prediv + 1U),
                                      UTILS_PLLInitStruct->PLLMul);
#else
  pllfreq = __LL_RCC_CALC_PLLCLK_FREQ(
    PLL_InputFrequency / ((UTILS_PLLInitStruct->Prediv >> RCC_CFGR_PLLXTPRE_Pos) + 1U),
    UTILS_PLLInitStruct->PLLMul);
                                             ...
                                             LL_UTILS_PLLInitTypeDef *UTILS_PLL2InitStruct)
{
  ...
  pll2freq = __LL_RCC_CALC_PLL2CLK_FREQ(PLL2_InputFrequency, UTILS_PLL2InitStruct->PLLMul,
                                        UTILS_PLL2InitStruct->Prediv);
                                                  ...
                                                  LL_UTILS_ClkInitTypeDef *UTILS_ClkInitStruct)
{
  ...
  sysclk_frequency_current =
    (SystemCoreClock << AHBPrescTable[LL_RCC_GetAHBPrescaler() >> RCC_CFGR_HPRE_Pos]);
#endif /* FLASH_ACR_LATENCY */
  ...
  if (status == SUCCESS)
  {
    LL_SetSystemCoreClock(
      __LL_RCC_CALC_HCLK_FREQ(SYSCLK_Frequency, UTILS_ClkInitStruct->AHBCLKDivider));
  }
